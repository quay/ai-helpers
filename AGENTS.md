...
## 6. Plugin script paths

Skills reference scripts using `.claude/scripts/<script-name>.sh` paths.
This is the consumer-facing path after plugin installation — do not use
bare relative paths like `scripts/foo.sh` or `${CLAUDE_PLUGIN_ROOT}/scripts/`.
