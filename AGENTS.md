...
## 7. Plugin documentation sync

When adding, removing, or modifying plugins, skills, or commands — or
changing `plugin.json` — run `make update` before committing. This
regenerates `PLUGINS.md` and `docs/data.json`. CI (`docs-sync`) will
fail if these files are stale.
