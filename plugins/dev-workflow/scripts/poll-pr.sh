...
#
# Usage: bash scripts/poll-pr.sh <PR_NUMBER> [OPTIONS]
#
# Options:
#   --repo OWNER/REPO        Repository (default: $DEFAULT_REPO or quay/quay)
...
#   4  Awaiting human review — posts review team mention; loops (--once: exits)
#
# Environment variables:
#   DEFAULT_REPO         — GitHub org/repo (default: quay/quay)
#   REVIEW_TEAM          — GitHub team to request reviews from (default: downstream)
#
...
notify_for_review() {
  ...
  gh api "repos/${REPO}/pulls/${PR_NUMBER}/requested_reviewers" \
    -X POST -f "team_reviewers[]=${REVIEW_TEAM}" >/dev/null 2>&1 \
    || echo "  NOTE: ${REVIEW_TEAM} team assignment skipped (not a repo collaborator)."
}
...
fetch_review_threads() {
  ...
  if [ "$has_next" = "true" ]; then
    echo "  WARN: PR has >100 review threads; results are truncated." >&2
  fi
  echo "$result"
}
...
do_poll() {
  ...
  if $FULL || $is_first; then
    ...
    if [ -n "$walkthrough_body" ] && [ "$walkthrough_body" != "null" ]; then
      echo "$walkthrough_body" | grep -E "(pass|fail|warn)" | head -20 || true
    else
...
while true; do
  ...
  case "$RC" in
    0)
      ...
      ;;
    1)
      echo "CI failures detected — fix and re-run."
      exit 1
      ;;
    3)
      echo "Review feedback requires attention — address and re-run."
      exit 3
      ;;
    2)
