...
# and blocks if it doesn't match the required format.
#
# Environment variables:
#   COMMIT_MESSAGE_PATTERN — regex for first line (default: ^[[:alnum:]_/.-]+: .+)
#   COMMIT_TICKET_FORMAT   — hint for expected ticket format (default: PROJQUAY-####|NO-ISSUE)

set -euo pipefail

: "${COMMIT_MESSAGE_PATTERN:=^[[:alnum:]_/.-]+: .+}"
: "${COMMIT_TICKET_FORMAT:=PROJQUAY-####|NO-ISSUE}"
...
if ! echo "$FIRST_LINE" | grep -qE "$COMMIT_MESSAGE_PATTERN"; then
  echo "BLOCKED: Commit message does not match required format." >&2
  echo "Expected: <subsystem>: <what changed> (${COMMIT_TICKET_FORMAT})" >&2
