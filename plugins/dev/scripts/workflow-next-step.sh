...
if command -v gh &>/dev/null; then
  PR_JSON=$(gh pr list --head "$BRANCH" --repo "${DEFAULT_REPO}" --json number,state --jq '.[0]' 2>/dev/null || true)
  ...
  if [ -n "$PR_NUM" ]; then
    ...
    if [ -f "$POLL_STATE" ]; then
      ...
      case "$LAST_EXIT" in
        ...
        2) msg="PR #${PR_NUM} has checks still pending. Re-run /poll ${PR_NUM} shortly." ;;
        3) msg="PR #${PR_NUM} has review comments to address. Fix and re-run /poll ${PR_NUM}." ;;
        4) msg="PR #${PR_NUM} is awaiting human review." ;;
        *) msg="PR #${PR_NUM} is open. Run /poll ${PR_NUM} to check status." ;;
      esac
    else
