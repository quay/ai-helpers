...
allowed-tools:
  ...
  - Bash(git push --force-with-lease fork *)
  - Bash(git reset *)
  - Bash(git reflog *)
  - Bash(gh pr view *)
  - Bash(gh pr checkout *)
...
Check out the PR branch locally and capture its tracking remote:

```bash
gh pr checkout "$ARGUMENTS"
CURRENT=$(git rev-parse --abbrev-ref HEAD)
TARGET="<baseRefName from above>"
REMOTE=$(git config "branch.$CURRENT.remote" 2>/dev/null || echo origin)
```
...
# Commits to rebase
git rev-list --count "$REMOTE/$TARGET..HEAD"

# Files changed on our branch
git diff --stat "$REMOTE/$TARGET...HEAD" | tail -1

# How far behind upstream
git rev-list --count "HEAD..$REMOTE/$TARGET"

# Check for merge commits (affects strategy)
git log --merges --oneline "$REMOTE/$TARGET..HEAD"

# Count files changed on BOTH sides (conflict indicators)
comm -12 \
  <(git diff --name-only "$(git merge-base HEAD $REMOTE/$TARGET)..HEAD" | sort) \
  <(git diff --name-only "$(git merge-base HEAD $REMOTE/$TARGET)..$REMOTE/$TARGET" | sort)
```
...
# Record pre-rebase diff for verification (three-dot: changes since merge base)
git diff "$REMOTE/$TARGET...HEAD" > /tmp/pre-rebase.diff
```
...
# git rebase --rebase-merges "$REMOTE/$TARGET"
```
...
```bash
git log --oneline --reverse "$REMOTE/$TARGET..HEAD"
```
...
MERGE_BASE=$(git merge-base HEAD "$REMOTE/$TARGET")
...
git log --oneline --reverse "$MERGE_BASE..$REMOTE/$TARGET" | head -n 30 | tail -1
git log --oneline --reverse "$MERGE_BASE..$REMOTE/$TARGET" | head -n 60 | tail -1
```
...
```bash
COMMITS=$(git rev-list --reverse "$REMOTE/$TARGET..HEAD")
git checkout -b "rebase-wip/$CURRENT" "$REMOTE/$TARGET"
...
```bash
git diff "$REMOTE/$TARGET...HEAD" > /tmp/post-rebase.diff
...
```bash
git push --force-with-lease "$REMOTE" "$CURRENT"
```

If rejected (remote ref moved since last fetch), run `git fetch "$REMOTE" "$CURRENT"`
