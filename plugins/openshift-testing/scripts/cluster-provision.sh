...
#   GANGWAY_BASE     — Gangway API base URL
#   GANGWAY_TOKEN    — Auth token for Gangway
#   GANGWAY_JOB_NAME — ProwJob name for cluster claims
#   KUBECONFIG_ENCRYPTION_KEY — Passphrase for kubeconfig decryption
...
: "${GANGWAY_JOB_NAME:=periodic-ci-quay-quay-master-claim-claim-cluster}"

JOB_NAME="${GANGWAY_JOB_NAME}"
...
check_prereqs() {
  if [[ -z "${GANGWAY_TOKEN:-}" ]]; then
    ...
    echo "  1. oc login https://api.ci.l2s4.p1.openshiftapps.com:6443 --web"
    echo "  2. export GANGWAY_TOKEN=\$(oc whoami -t)"
    exit 1
  fi
  if [[ -z "${ENCRYPTION_KEY}" ]]; then
    echo "KUBECONFIG_ENCRYPTION_KEY is not set."
    echo "Contact the CI team for the decryption passphrase."
    exit 1
  fi
...
gangway_post() {
  ...
  if ! http_code=$(curl -s -w "%{http_code}" -o "$tmpfile" \
    ...
    -X POST -H "Authorization: Bearer ${GANGWAY_TOKEN}" \
    -H "Content-Type: application/json" -d "$payload" \
    "${GANGWAY_BASE}${endpoint}"); then
    body=$(cat "$tmpfile" 2>/dev/null || true); rm -f "$tmpfile"
    die "Gangway API POST ${endpoint} transport error: ${body}"
  fi
  body=$(cat "$tmpfile"); rm -f "$tmpfile"
  if [[ "$http_code" == "401" ]]; then
    die "Authentication failed (HTTP 401). Re-run: oc login ... && export GANGWAY_TOKEN=\$(oc whoami -t)"
  fi
...
gangway_get() {
  ...
  for attempt in 1 2 3; do
    ...
    if [[ "$http_code" == "429" ]]; then
      info "Rate-limited (429) — backing off 90s (attempt ${attempt}/3)..."
      sleep 90; continue
    fi
    if [[ "$http_code" == "401" ]]; then
      die "Authentication failed (HTTP 401). Re-run: oc login ... && export GANGWAY_TOKEN=\$(oc whoami -t)"
...
save_state() {
  ...
  jq -n --arg eid "$1" --arg gcs "$2" --arg url "$3" --arg kfg "$4" --arg sta "$5" --arg sts "$6" \
...
update_state_field() {
  ...
  if [[ -f "$STATE_FILE" ]]; then
    [[ -L "$STATE_DIR" ]] && die "Refusing to write through symlinked state dir: $STATE_DIR"
    [[ -L "$STATE_FILE" ]] && die "Refusing to write through symlinked state file: $STATE_FILE"
    local tmpfile; tmpfile=$(mktemp "${STATE_DIR}/.state.XXXXXX")
    jq --arg f "$field" --arg v "$value" '.[$f] = $v' "$STATE_FILE" >"$tmpfile"
    mv "$tmpfile" "$STATE_FILE"
  fi
}

gcs_from_url() { echo "$1" | sed -n 's|.*/view/gs/||p'; }
...
try_download_kubeconfig() {
  ...
  if ! response_meta=$(curl -s -w "%{http_code}|%{content_type}" -o "$tmpfile" \
    --connect-timeout "$CURL_CONNECT_TIMEOUT" --max-time 60 "$url"); then
    rm -f "$tmpfile"; return 1
  fi
  http_code="${response_meta%%|*}"; content_type="${response_meta#*|}"
  if [[ "$http_code" == "200" ]] && [[ "$content_type" != *"text/html"* ]] && [[ -s "$tmpfile" ]]; then
    mkdir -p "$(dirname "$KUBECONFIG_OUT")"
    if openssl enc -d -aes-256-cbc -pbkdf2 -pass pass:"${ENCRYPTION_KEY}" \
      -in "$tmpfile" -out "${KUBECONFIG_OUT}"; then
      chmod 600 "$KUBECONFIG_OUT"; rm -f "$tmpfile"; return 0
    else
      echo "WARNING: Failed to decrypt kubeconfig." >&2
      rm -f "$tmpfile"; return 1
    fi
  fi
  rm -f "$tmpfile"; return 1
...
ensure_oc() {
  if command -v oc >/dev/null 2>&1; then return 0; fi
  info "oc not found — installing..."
  local arch tmpdir tarball
  arch=$(uname -m)
  case "$arch" in x86_64) arch="amd64" ;; aarch64) arch="arm64" ;; *) echo "WARNING: unsupported arch $arch"; return 1 ;; esac
  tmpdir=$(mktemp -d); tarball="${tmpdir}/oc.tar.gz"
  if curl -sL --connect-timeout "$CURL_CONNECT_TIMEOUT" --max-time 120 -o "$tarball" \
    ...
    if [[ -x "${tmpdir}/oc" ]]; then
      mv "${tmpdir}/oc" /usr/local/bin/oc 2>/dev/null || mv "${tmpdir}/oc" "${HOME}/.local/bin/oc" 2>/dev/null || {
        export PATH="${tmpdir}:${PATH}"; info "oc installed to ${tmpdir}/oc"; return 0
      }
...
validate_cluster() {
  ensure_oc || true
  if ! command -v oc >/dev/null 2>&1; then echo "WARNING: could not install 'oc'"; return; fi
  local server
  server=$(oc --kubeconfig="$KUBECONFIG_OUT" whoami --show-server 2>/dev/null || true)
  if [[ -n "$server" ]]; then info "Cluster validated — server: ${server}"; else
    echo "WARNING: Could not validate cluster connectivity."
  fi
}

poll_and_download() {
  local exec_id="$1" job_url="$2"
  local gcs_root start_epoch interval status elapsed response live_url
  gcs_root=$(gcs_from_url "$job_url")
  start_epoch=$(epoch_now); interval=$POLL_INTERVAL_INITIAL
  info "Polling for cluster readiness (timeout: ${POLL_TIMEOUT}s)..."
  while true; do
    elapsed=$(($(epoch_now) - start_epoch))
    if ((elapsed > POLL_TIMEOUT)); then die "Timed out after ${POLL_TIMEOUT}s. Check job: ${job_url}"; fi
    ...
    if [[ -z "$gcs_root" ]]; then
      ...
      if [[ -n "$live_url" ]]; then job_url="$live_url"; gcs_root=$(gcs_from_url "$job_url"); update_state_field "job_url" "$job_url"; info "Job URL: ${job_url}"; fi
    fi
    update_state_field "status" "$status"
    case "$status" in FAILURE|ERROR|ABORTED) die "Job ended with status ${status}. Check: ${job_url}" ;; esac
    if [[ -n "$gcs_root" ]] && try_download_kubeconfig "$gcs_root"; then
      ...
      echo "Kubeconfig: ${KUBECONFIG_OUT}"; echo "Job URL:    ${job_url}"; echo "Note:       Cluster auto-expires in ~4 hours"
      return 0
    fi
    info "Status: ${status} | Elapsed: ${elapsed}s | Next check in ${interval}s"
    sleep "$interval"
    if ((interval < POLL_INTERVAL_MAX)); then interval=$POLL_INTERVAL_MAX; fi
  done
}
...
up() {
  check_prereqs
  if [[ -f "$STATE_FILE" ]]; then
    [[ -L "$STATE_FILE" ]] && die "Refusing to read symlinked state file"
    ...
    if [[ -n "$existing_id" && "$existing_status" != "FAILURE" && "$existing_status" != "ERROR" && "$existing_status" != "ABORTED" ]]; then
      ...
      poll_and_download "$existing_id" "$existing_job_url"; return $?
    fi
  fi
  info "Triggering cluster claim via Gangway..."
  local payload response exec_id gcs_path job_url job_status
  payload=$(jq -n '{job_name: $job, job_execution_type: "1", pod_spec_options: { envs: { OCP_VERSION: $ocp } }}' --arg job "$JOB_NAME" --arg ocp "$OCP_VERSION")
  ...
  if [[ -z "$exec_id" ]]; then die "Failed to get execution ID: ${response}"; fi
  info "Execution triggered: ${exec_id}"; info "Job URL: ${job_url}"
...
down() {
  ...
  if [[ -f "$STATE_FILE" ]]; then
    [[ -L "$STATE_FILE" ]] && die "Refusing to read symlinked state file"
    local kubeconfig_path; kubeconfig_path=$(jq -r '.kubeconfig_path // empty' "$STATE_FILE")
    info "Cleaning up state..."; rm -f "$STATE_FILE"
    if [[ -n "$kubeconfig_path" && -f "$kubeconfig_path" ]]; then rm -f "$kubeconfig_path"; info "Removed kubeconfig: ${kubeconfig_path}"; fi
    rmdir "$STATE_DIR" 2>/dev/null || true; info "Cleanup complete."
  else info "No active cluster provision found."; fi
  echo ""; echo "Note: Clusters auto-expire after ~4 hours via Hive."
}

status() {
  command -v jq >/dev/null 2>&1 || die "'jq' is required"
  if [[ ! -f "$STATE_FILE" ]]; then echo "No active cluster provision."; return 0; fi
  [[ -L "$STATE_FILE" ]] && die "Refusing to read symlinked state file"
  local state exec_id started_at cur_status kubeconfig_path job_url
  ...
  echo "Execution ID:  ${exec_id}"; echo "Started:       ${started_at}"
  echo "Cached Status: ${cur_status}"; echo "Job URL:       ${job_url}"
  echo "Kubeconfig:    ${kubeconfig_path}"
  if [[ "$started_at" != "unknown" ]]; then
    local start_epoch now_epoch remaining_s remaining_m
    start_epoch=$(epoch_from_iso "$started_at"); now_epoch=$(epoch_now)
    remaining_s=$((14400 - (now_epoch - start_epoch)))
    if ((remaining_s > 0)); then remaining_m=$((remaining_s / 60)); echo "Est. remaining: ~${remaining_m} minutes"
    else echo "Est. remaining: EXPIRED"; fi
  fi
  if [[ -f "$kubeconfig_path" ]]; then
    ensure_oc
    if command -v oc >/dev/null 2>&1; then
      echo ""; info "Testing cluster connectivity..."
      local server; server=$(oc --kubeconfig="$kubeconfig_path" whoami --show-server 2>/dev/null || true)
      if [[ -n "$server" ]]; then echo "Cluster: ${server} (reachable)"; else echo "Cluster: NOT reachable"; fi
    fi
  fi
}
