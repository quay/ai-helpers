...
func planBackports(state *ActorState) {
	...
	requestNextCherryPick(state)
}

func requestNextCherryPick(state *ActorState) {
	...
	for _, branch := range state.Backports.RequiredBranches {
		...
		if ghClient != nil && pr.Number > 0 {
			...
			if err := ghClient.PostPRComment(ctx, pr.Repo, pr.Number, comment); err != nil {
...
func handleBackportMerged(state *ActorState) {
	if state.Backports == nil {
		return
	}
...
// versionToBranch converts "3.12.1" → "redhat-3.12"
