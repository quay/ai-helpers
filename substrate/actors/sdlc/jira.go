...
func ProcessJIRAEvent(state *ActorState, payload []byte) (string, error) {
	...
	case "assignee_changed":
		return handleAssigneeChange(state, &event)
	case "description_updated":
		return handleFieldUpdate(state, &event)
	default:
...
func handleAssigneeChange(state *ActorState, event *jiraWebhookEvent) (string, error) {
	...
	return applyTriageResult(state)
}

func handleFieldUpdate(state *ActorState, event *jiraWebhookEvent) (string, error) {
...
func applyTriageResult(state *ActorState) (string, error) {
	...
	if jiraClient != nil {
		...
		if err := jiraClient.AddComment(state.Ticket, comment); err != nil {
