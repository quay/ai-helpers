...
func runReviewFeedbackChain(state *ActorState) {
	...
	err := claudeClient.AddressFeedback(ctx, state.JIRA, pr)
	if err != nil {
...
func handleClarificationReply(state *ActorState, commentBody string) (string, error) {
	if state.Phase != PhaseClarificationNeeded {
		return "ignored (not in clarification phase)", nil
	}

	resolved, err := claudeClient.CheckClarification(context.Background(), commentBody)
	if err != nil {
...
func categorizeAndTransition(state *ActorState, review *parsedReview) {
	pr := findActivePR(state)
	if pr == nil {
		return
	}
	...
	if hasAmbiguous {
		...
		if pr != nil {
			...
			if ghClient != nil && pr.Number > 0 {
				ghClient.PostPRComment(context.Background(), pr.Repo, pr.Number,
					"I have questions about some review comments. Could you clarify the ambiguous feedback?")
			}
		}
	} else {
		state.Phase = PhaseAddressingFeedback
