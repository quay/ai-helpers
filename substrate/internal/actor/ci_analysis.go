...
func runCIAnalysisChain(h *Handler, state *ActorState) {
	...
	analysis, err := h.claude.AnalyzeCI(ctx, state.JIRA, failingChecks)
	if err != nil {
	...
	if !analysis.Fixable || analysis.Confidence < 0.7 {
		...
		postCIBlockedComment(ctx, h, state, pr, analysis)
		return
	}
	...
	if err := h.claude.FixCI(ctx, analysis); err != nil {
...
func postCIBlockedComment(ctx context.Context, h *Handler, state *ActorState, pr *PRState, analysis *CIAnalysisResult) {
	...
	if err := h.github.PostPRComment(ctx, pr.Repo, pr.Number, sb.String()); err != nil {
