...
```bash
git push "$PUSH_REMOTE" "$FIX_BRANCH"

PR_BODY=$(cat <<'PREOF'
## Summary

This PR fixes **${CVE_ID}** by updating ${PACKAGE} from ${INSTALLED_VERSION} to ${FIXED_VERSION}.
...
### Assessment
See assessment artifact for full analysis including advisory sources and
impact analysis.
- **Category**: ${VERDICT}
- **Evidence**: ${EVIDENCE}
...
### Verification
- Post-fix scan: ${SCAN_RESULT}

### Checklist
- [x] CVE assessed and verified as affecting this branch
- [x] Pre-PR tests executed
- [ ] Verify CVE is resolved with security scan
- [ ] Review test failures (if any)

---
Generated by Quay CVE Fix Workflow
<!-- quay-cvefix-workflow -->
PREOF
)
...
PR_URL=$(gh pr create \
  --repo "$UPSTREAM_REPO" \
  --base "$GIT_BRANCH" \
  --head "${PR_HEAD_PREFIX}${FIX_BRANCH}" \
  --title "fix(cve): ${CVE_ID} - ${PACKAGE} (${JIRA_KEY})" \
  --body "$PR_BODY" 2>&1)

echo "PR created: ${PR_URL}"
```

### 11. Post Jira Comment

```bash
COMMENT="[Phase: Fix] CVE Remediation PR Created

CVE: ${CVE_ID}
Package: ${PACKAGE} ${INSTALLED_VERSION} -> ${FIXED_VERSION}
Branch: ${FIX_BRANCH}

Fix Applied: $(echo "$FIX_DESCRIPTION")
Test Results: ${TEST_STATUS}
Post-fix Scan: ${SCAN_RESULT}

PR: ${PR_URL}
Backport Required: $([ "$GIT_BRANCH" = "master" ] && echo "yes — redhat-${TARGET_VERSION}" || echo "no")

Next: PR ready for review."

bash .claude/scripts/jira-ops.sh comment "$JIRA_KEY" "$COMMENT"
```

### 12. Write Fix Report

Save to `artifacts/quay-cvefix/fixes/fix-implementation-${CVE_ID}.md`:
...
- `artifacts/quay-cvefix/fixes/fix-implementation-CVE-YYYY-XXXXX.md` — per-CVE fix report
...
- Pull requests created on GitHub (one per CVE per branch)
- Jira comments posted to each ticket
...
- [ ] Jira comment posted with PR URL and test results
